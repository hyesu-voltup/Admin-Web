...
import { budgetQueryKey } from "../hooks/useBudget";
 ...
 * - 참여 취소: POST .../cancel (포인트 회수), C015 시 특수 경고
 */
export default function RouletteParticipationsSection() {
  const queryClient = useQueryClient();
  const { data: participations = [], isLoading, isError, error } =
    useRouletteParticipations();
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("참여가 취소되었습니다. 포인트가 회수되었습니다.");
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">룰렛 참여 취소 (포인트 회수)</h3>
